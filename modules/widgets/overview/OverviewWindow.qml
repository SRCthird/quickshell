...
Item {
    ...
    // Timer to reset override position after a delay (waiting for HyprctlService update)
    Timer {
    ...
    MouseArea {
        ...
        onEntered: {
            ...
            if (root.windowData) {
                // Get current active workspace from HyprctlService
                let currentWorkspace = HyprctlService.focusedMonitor?.activeWorkspace?.id;

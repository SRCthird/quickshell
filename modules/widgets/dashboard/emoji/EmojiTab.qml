...
Rectangle {
    ...
    function loadRecentEmojis() {
        recentProcess.command = ["bash", "-c", "cat " + Quickshell.cachePath("emojis.json") + " 2>/dev/null || echo '[]'"];
        recentProcess.running = true;
    }

    function saveRecentEmojis() {
        var jsonData = JSON.stringify(recentEmojis, null, 2);
        saveProcess.command = ["bash", "-c", "echo '" + jsonData.replace(/'/g, "'\\''") + "' > " + Quickshell.cachePath("emojis.json")];

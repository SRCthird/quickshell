...
PanelWindow {
    ...
    property alias thumbnailsVersion: thumbnailManager.version

    Thumbnail {
        id: thumbnailManager

        wallpaperPaths: wallpaper.wallpaperPaths
        wallpaperDir: wallpaper.wallpaperDir
    ...
    function requestThumbnailGeneration() {
        ...
        thumbnailManager.requestGeneration();
    }

    function scheduleThumbnailGeneration() {
        if (GlobalStates.wallpaperManager
                && GlobalStates.wallpaperManager !== wallpaper) {
            GlobalStates.wallpaperManager.scheduleThumbnailGeneration();
            return;
        }

        thumbnailManager.scheduleGeneration();
    }

    function getThumbnailPath(filePath) {
        return thumbnailManager.getThumbnailPath(filePath);
    }
    ...
    function getLockscreenFramePath(filePath) {
        return lockscreenManager.getFramePath(filePath);
    }

    function generateLockscreenFrame(filePath) {
        lockscreenManager.generate(filePath);
    }

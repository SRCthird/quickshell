...
Item {
    ...
    property bool isEditingShell: false
    ...
    function openEditDialog(bind, index, isShell) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingShell = isShell;

        // Initialize edit form state
        if (isShell) {
            // Shell binds still use old format (single key)
    ...
    function saveEdit() {
        if (root.isEditingShell) {
            // Save shell bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["shell", "section"?, "bindName"]
            
            const adapter = Config.keybinds;
            if (adapter && adapter.shell) {
                let bindObj = null;
                if (path.length === 2) {
                    // Top level: shell.bindName
                    bindObj = adapter.shell[path[1]];
                } else if (path.length === 3) {
                    // Nested: shell.system.bindName
                    bindObj = adapter.shell[path[1]][path[2]];
                }
            ...
            const customBinds = Config.keybinds.custom || [];
            ...
            Config.keybinds.custom = newBinds;
        } else {
    ...
    // Get shell binds as a flat list
    function getShellBinds() {
        const adapter = Config.keybinds;
        if (!adapter || !adapter.shell)
            return [];
        ...
        // Core Shell binds (Launcher, Dashboard, etc.)
            ...
            for (const key of systemKeys) {
                if (shell.system[key]) {
                    binds.push({
                        category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "shell.system." + key,
                        bind: shell.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            Repeater {
                id: shellRepeater
                model: root.currentCategory === "shell" ? root.getShellBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "shell" && shellRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "shell" ? "No Shell binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingShell && !root.isCreatingNew
                        ...
                        // Reset button (only for Shell binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingShell
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingShell && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["shell", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        
                                        // Use the new helper in Config.qml to get the default values
                                        const defaultBind = Config.keybinds.getShellDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for shell binds only)
                        Text {
                            visible: root.isEditingShell && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingShell
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for shell)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingShell - Removed to allow editing flags for Shell binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingShell
                            ...
                            StyledRect {
                                ...
                                radius: Styling.radius(-2)
                                opacity: root.isEditingShell ? 0.6 : 1.0
                            ...
                            // =====================
                            // LAYOUT SELECTOR (for HyprctlService)
                            // =====================
                            Text {
                                text: "Layouts (HyprctlService)"
                                font.family: Config.theme.font
                                font.pixelSize: Styling.fontSize(-1)
                                font.weight: Font.Medium
                                color: Colors.overSurfaceVariant

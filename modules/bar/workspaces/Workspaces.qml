...
Item {
    ...
    readonly property var monitor: HyprctlService.monitorFor(bar.screen)
    ...
    function updateWorkspaceOccupied() {
        if (Config.workspaces.dynamic) {
            const occupiedIds = HyprctlService.workspaces.values.filter(ws => CompositorData.workspaceOccupationMap[ws.id]).map(ws => ws.id).sort((a, b) => a - b).slice(0, Config.workspaces.shown);
    ...
    Connections {
        target: HyprctlService.workspaces

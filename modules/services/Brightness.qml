...
Singleton {
    ...
    function increaseBrightness(): void {
        const focusedName = HyprctlService.focusedMonitor.name;
    ...
    IpcHandler {
        ...
        property bool hasPreviousBrightnessSnapshot: false

        function saveBrightnessSnapshot(monitorName: string): void {
            const snapshot = {};
            ...
            if (!monitorName || monitorName === "") {
                ...
                for (let i = 0; i < root.monitors.length; ++i) {
                    const mon = root.monitors[i];
                    if (mon && mon.ready && mon.screen && mon.screen.name) {
                        snapshot[mon.screen.name] = mon.brightness;
                    }
                }
            } else {
                // Save specific monitor
                const monitor = root.monitors.find(m =>
                    m && m.screen && m.screen.name === monitorName
                );

                if (monitor && monitor.ready) {
                    snapshot[monitor.screen.name] = monitor.brightness;
                } else {
            ...
            if (Object.keys(snapshot).length === 0) {
                console.warn("No monitor brightness values available to save");
            }

            previousBrightnessSnapshot = snapshot;
            hasPreviousBrightnessSnapshot = true;
        }

        function clearBrightnessSnapshot() {
            previousBrightnessSnapshot = ({});
            hasPreviousBrightnessSnapshot = false;
        }

        function increment() {
            saveBrightnessSnapshot("")
        ...
        function set(value: real, monitorName: string) {
            saveBrightnessSnapshot(monitorName)
        ...
        function revert(monitorName: string) {
            if (!hasPreviousBrightnessSnapshot) {
                console.warn("No previous brightness snapshot to revert to");
                return;
            }

            if (!monitorName || monitorName === "") {
                // Revert all monitors from last snapshot
                const names = Object.keys(previousBrightnessSnapshot);

                for (let i = 0; i < names.length; ++i) {
                    const name = names[i];
                    const value = previousBrightnessSnapshot[name];
                    const monitor = root.monitors.find(m =>
                        m && m.screen && m.screen.name === name
                    );

                    if (monitor && monitor.ready) {
                        monitor.setBrightness(value);
                    } else {
                        console.warn("Monitor not found or not ready during revert:", name);
                    }
                }

                clearBrightnessSnapshot();
            } else {
                // Revert specific monitor from last snapshot
                if (!(monitorName in previousBrightnessSnapshot)) {
                    console.warn("No saved brightness found for monitor:", monitorName);
                    return;
                }
                ...
                if (monitor && monitor.ready) {
                    monitor.setBrightness(previousBrightnessSnapshot[monitorName]);

                    // Remove just this monitor from the snapshot
                    delete previousBrightnessSnapshot[monitorName];

                    if (Object.keys(previousBrightnessSnapshot).length === 0) {
                        clearBrightnessSnapshot();
                    }
                } else {
                    console.warn("Monitor not found or not ready:", monitorName);
